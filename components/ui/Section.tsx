...
export const Section = ({title, children, className = ''}: SectionProps) => {
    ...
    return (
        <View className={className}>
            {title && (
                <Text className="mb-3 px-1 text-sm font-semibold" style={{color: colors.primary}}>
                    {title}
                </Text>
            )}
            {children}
...
export const SectionHeader = ({title, subtitle, action, className = ''}: SectionHeaderProps) => {
    ...
    return (
        <View className={`mb-3 flex-row items-start justify-between ${className}`}>
            <View className="flex-1">
                <Text className="text-base font-semibold" style={{color: colors.onSurface}}>
                    {title}
                </Text>
                {subtitle && (
                    <Text className="mt-0.5 text-sm" style={{color: colors.onSurfaceVariant}}>
                        {subtitle}
                    </Text>
                )}
            </View>
...
export const Divider = ({className = ''}: DividerProps) => {
    ...
    return <View className={`h-px ${className}`} style={{backgroundColor: colors.outlineVariant}} />
}
