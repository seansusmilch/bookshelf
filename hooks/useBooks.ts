...
export const useBooks = (statusFilter?: string) => {
    const books = useQuery(api.books.getUserBooks, statusFilter ? {status: statusFilter} : {})
...
export const useBookDetail = (bookId: string | null) => {
    return useQuery(api.books.getBookById, bookId ? {bookId: bookId as Id<'books'>} : 'skip')
}
...
export const useBookIdByOpenLibraryId = (openLibraryId: string | null | undefined) => {
    const bookId = useQuery(
        api.books.getBookIdByOpenLibraryId,
        openLibraryId ? {openLibraryId} : 'skip'
    )
