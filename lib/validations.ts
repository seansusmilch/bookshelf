...
export const bookSchema = z.object({
  ...
  totalPages: z.number().int().positive('Total pages must be positive').min(1, 'Must have at least 1 page'),
  isbn: z.string().optional().refine((val) => {
    if (!val) return true;
...
export const yearlyGoalSchema = z.object({
  booksGoal: z.number().int().positive('Goal must be positive').min(1, 'Goal must be at least 1 book'),
  pagesGoal: z.number().int().positive('Goal must be positive').min(1, 'Goal must be at least 1 page').optional(),
});
