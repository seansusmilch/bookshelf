...
export default function AddBookScreen() {
    ...
    const {bookDetails, isLoading, error, workTitle, initialCoverUrl, initialAuthor, reload} =
        useBookEditions({
            workId,
            authorFallback,
            coverUrlFallback,
    ...
    if (error || !bookDetails) {
        return <PageError message={error || 'Failed to load book'} onRetry={reload} />
    }

    const hasMetaInfo =
        bookDetails.pageCount || bookDetails.publishDate || bookDetails.publishers?.[0]?.name
